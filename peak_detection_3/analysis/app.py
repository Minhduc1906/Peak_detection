...
# ===========================================
# NEW: Throughput Calculation Functions (MATLAB-compatible method)
# ===========================================

def calculate_rate_matlab_method(df, byte_col, time_col='relative_time'):
    """
    Calculate throughput using MATLAB method - byte values are already rates per interval
    MATLAB: 8 * ds(2:end, 5) / ds_interval / 1e6
    """
    if byte_col not in df.columns:
        return np.zeros(len(df))
    
    # Skip first row and calculate mean interval (like MATLAB)
...
def calculate_packet_rate_matlab_method(df, packet_col, time_col='relative_time'):
    """
    Calculate packet rate using MATLAB method - packet values are already rates per interval
    MATLAB: ds(2:end, 4) / ds_interval
    """
    ...
    # Calculate mean interval like MATLAB
...
def predict_with_lstm(df, model_data, model_name):
    ...
    # required_features = ["peak_score", "peak_lt", "peak_st", "filtered_throughput", "ratio_st_lt"]
...
def create_throughput_figures(df, is_downstream=True, show_peak_detection=False, 
    ...
    # Calculate rates using the MATLAB-compatible method (matches original MATLAB code)
    tx_packet_rate = calculate_packet_rate_matlab_method(df, 'tx_packets', 'relative_time')
    rx_packet_rate = calculate_packet_rate_matlab_method(df, 'rx_packets', 'relative_time')
    tx_bit_rate = calculate_rate_matlab_method(df, 'tx_bytes', 'relative_time')
    rx_bit_rate = calculate_rate_matlab_method(df, 'rx_bytes', 'relative_time')
    ...
    if 'tx_bytes' in df.columns and 'rx_bytes' in df.columns:
        ...
        # Layout
        fig.update_layout(font=dict(size=18),
            title=f'{direction} Throughput (Mb/s) - MATLAB-compatible Calculation',
...
def data_visualization_page(downstream_df, upstream_df):
    ...
    # Calculate rate statistics using MATLAB-compatible method
    if 'tx_bytes' in downstream_df.columns and 'rx_bytes' in downstream_df.columns:
        # Use MATLAB-compatible calculation
        ds_tx_bit_rate = calculate_rate_matlab_method(downstream_df, 'tx_bytes', 'relative_time')
        ds_rx_bit_rate = calculate_rate_matlab_method(downstream_df, 'rx_bytes', 'relative_time')
        
        col1, col2 = st.columns(2)
        with col1:
            st.write("**Downstream Statistics (MATLAB-compatible Calculation)**")
        ...
        if 'tx_bytes' in upstream_df.columns and 'rx_bytes' in upstream_df.columns:
            # Use MATLAB-compatible calculation (same as downstream)
            us_tx_bit_rate = calculate_rate_matlab_method(upstream_df, 'tx_bytes', 'relative_time')
            us_rx_bit_rate = calculate_rate_matlab_method(upstream_df, 'rx_bytes', 'relative_time')
            
            with col2:
                st.write("**Upstream Statistics (MATLAB-compatible Calculation)**")
...
def create_peak_detection_visualization(time_array, throughput, network_capacity, 
    ...
    """Create a detailed peak detection visualization using Plotly (similar to original Graph 11)"""
    
    # NOTE: throughput is calculated using MATLAB-compatible method for accuracy and consistency with original implementation
...
def detection_comparison_page(downstream_df, downstream_features_df, lstm_models):
    ...
    with st.spinner("Calculating traditional peak detection..."):
        # Prepare data for peak detection using the MATLAB-compatible method
        ds_time = downstream_df['relative_time'].values
        
        # Calculate throughput rates using MATLAB-compatible method
        if 'tx_bytes' in downstream_df.columns:
            # Use MATLAB-compatible calculation
            ds_tx_rate = calculate_rate_matlab_method(downstream_df, 'tx_bytes', 'relative_time')
    ...
    # NEW: Add detailed peak detection visualization
        ...
        if peak_viz_fig:
            ...
            if use_median_filter:
                st.markdown("""
                    ...
                    *Note: Throughput calculated using MATLAB-compatible method (raw byte values / mean interval) for consistency with original implementation*
                    """)
            else:
                st.markdown("""
                    **Understanding the Peak Detection Visualization (WITHOUT Median Filtering):**
                    - **Red Line**: **Raw** downstream throughput calculated using MATLAB-compatible method
                    - **Black Dashed Line**: Network capacity limit 
    ...
    # Downsample the data for other visualizations
    downsample_step = max(1, len(time_array) // 1000)  # Limit to ~1000 points for performance
    time_array_downsampled = time_array[::downsample_step]
    queue_exists_downsampled = queue_exists[::downsample_step]
    ds_peak_diff_downsampled = ds_peak_diff[::downsample_step]
    ds_tx_rate_downsampled = ds_tx_rate[::downsample_step]
    ds_filtered_downsampled = ds_filtered[::downsample_step]  # NEW: Add downsampled filtered throughput
    
    if downstream_predictions is not None:
        downstream_predictions_downsampled = downstream_predictions[::downsample_step]
    else:
    ...
    # NEW: Figure 1 - Ground Truth Downstream Throughput (Median Filtered from peak detection)
    st.subheader("Ground Truth - Downstream Throughput (Median Filtered)")
    ...
    fig_throughput.update_layout(
        title="Ground Truth - Downstream Throughput (Median Filtered)",
        xaxis_title="Time (s)",
        yaxis_title="Throughput (Mbps)",
        height=500,
        template='plotly_white',
    ...
    # NEW: Figure 2 - Throughput During Predicted Congestion (Both Methods) with filtering options
    if downstream_predictions_downsampled is not None:
        st.subheader("Throughput During Predicted Congestion")
        ...
        if show_disagreement:
            title_text = "Areas of Disagreement Between Detection Methods"
        else:
            title_text = "Throughput During Predicted Congestion"
        
        subtitle = f"<span style='font-size:14px'>Traditional Parameters: LW:{long_window_ms}ms, SW:{short_window_ms}ms, PT:{peak_tolerance}, CT:{count_threshold}</span>"
        full_title = f"{title_text}<br>{subtitle}"
    ...
    if 'queue_size' in downstream_df.columns:
        ...
        queue_size_downsampled = queue_size[::downsample_step]
        ...
        fig_ground_truth.update_layout(
            title="Ground Truth - Actual Buffer Occupancy",
            xaxis_title="Time (s)",
    ...
    fig_traditional.update_layout(
        title=f"Traditional Peak Detection - Binary Classification (LW: {long_window_ms}ms, SW: {short_window_ms}ms, PT: {peak_tolerance}, CT: {count_threshold})",
        xaxis_title="Time (s)",
    ...
    if downstream_predictions_downsampled is not None:
        ...
        fig_lstm.update_layout(
            title="LSTM Prediction - Queue Probability",
            xaxis_title="Time (s)",
    ...
    if figures_for_export:
        ...
        st.markdown("---")
        st.info("Click the download links above to save individual graphs as PDF files for your paper.")
    ...
    if downstream_predictions is not None and 'queue_exists' in downstream_df.columns:
        ...
        binary_downstream_predictions = (downstream_predictions >= 0.5).astype(int)
        
        # Calculate metrics for both methods
        traditional_cm, traditional_report = calculate_lstm_metrics(queue_exists, ds_peaking.astype(int))
        lstm_cm, lstm_report = calculate_lstm_metrics(queue_exists, binary_downstream_predictions)
        ...
        ### Note on Throughput Calculation
        Both methods now use the **MATLAB-compatible calculation** where byte values are treated as rates per interval. 
        This matches the original MATLAB implementation and prevents negative throughput values.
        """)
...
def main():
    ...
    st.info("Throughput Calculation: Using MATLAB-compatible method where byte values are treated as rates per interval. This matches the original MATLAB implementation and eliminates negative throughput values.")
